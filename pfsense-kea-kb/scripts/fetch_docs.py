#!/usr/bin/env python3
"""Fetch and clean pfSense KEA DHCP documentation into local Markdown.

Reads sources.tsv (url <TAB> category <TAB> priority <TAB> pfsense_version_notes),
fetches each URL, strips site chrome, and writes a clean Markdown file under
docs/<category>/<slug>.md with YAML frontmatter.
...
Usage:
  ...
  python3 fetch_docs.py --only upstream-kea
...
def main() -> None:
    ...
    for row in rows:
        ...
        outpath = os.path.join(outdir, slugify(url) + ".md")
